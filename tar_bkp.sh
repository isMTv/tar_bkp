...
create_db_bkp () {
    while [ "$opt_m" = "true" ]; do
        ...
        cd "${source}" || exit 1
        local name_bkp="${project}-${curdate}.sql.gz.md5"
        ...
        bkp_hash="$(cat "${db_bkp}"/"${name_bkp}" 2> /dev/null)"
        if [ "$db_hash" != "$bkp_hash" ]; then
            rm -f "${db_bkp}"/"${project}"-*.sql.gz
            mysqldump -u "${db_user}" -p"${db_pass}" "${db_name}" | gzip > "${db_bkp}"/"${name_bkp::-4}" | sha1sum | awk '{print $1}' > "${db_bkp}"/"${name_bkp}" || exit 1 ; status_cdb="$?"
            local size_bkp="$(du -sh "${db_bkp}"/"${name_bkp::-4}" | awk '{print $1}')"
            if [ "$status_cdb" = "0" ] ; then logger "[+] [@create_db_bkp] Резервная копия (${name_bkp::-4}) создана [$size_bkp]." ; fi
            break
        else
            local size_bkp="$(du -sh "${db_bkp}"/"${project}"-*.sql.gz | awk '{print $1}')"
            logger "[-] [@create_db_bkp] В резервной копии (${name_bkp::-4}) изменений не обнаружено [$size_bkp]."
            break
        fi
    done
}
